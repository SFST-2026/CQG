...
## Definition
Let X_base(r) denote the solver’s dimensionless internal variable(s) for the baseline run and X_σ(r) the same variable with the σχ insertion. I define the pointwise amplification

WFaktor(r) = |X_σ(r) − X_base(r)| / max(ε_machine, |X_base(r)|).

I report a single run-level WFaktor as the maximum over the resolved domain:
...
## Computation steps
1. I compute baseline and inserted profiles on the same grid and with identical solver tolerances. 
2. I evaluate X_base(r) and X_σ(r) at grid points; if grids differ I use the same interpolation both ways. 
3. I use a small floor ε_machine (default 1e−16) to avoid division by zero. 
4. I store WFaktor_max and r_max, and I save WFaktor(r) as a profile plot.
